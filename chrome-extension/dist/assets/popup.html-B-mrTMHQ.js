(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`https://pocket-option-signal-api-production.up.railway.app`;async function t(){let t=await fetch(`${e}/signal`);if(!t.ok)throw Error(`Unable to load signal`);return await t.json()}async function n(t){let n=encodeURIComponent(t);console.log(`Loading candles for:`,t);let r=await fetch(`${e}/candles/${n}`);if(console.log(`Status:`,r.status),!r.ok)throw Error(`Unable to load candles`);let i=await r.json();return console.log(`Candles returned:`,i.length),i}function r(e){let t=Number(e??0),n=document.getElementById(`confidence`),r=document.getElementById(`gauge`);if(!n){console.error(`Missing element: confidenceText`);return}if(!r)return;n.innerHTML=`${t}%`;let i=377-t/100*377;r.style.strokeDashoffset=i,t>=80?r.style.stroke=`#22C55E`:t>=60?r.style.stroke=`#FACC15`:r.style.stroke=`#EF4444`}async function i(){try{let e=await(await fetch(`https://pocket-option-signal-api-production.up.railway.app/trade/statistics`)).json(),t=document.getElementById(`winRate`),n=document.getElementById(`wins`),r=document.getElementById(`losses`),i=document.getElementById(`profit`),a=document.getElementById(`accuracy`);t&&(t.innerHTML=`${e.win_rate}%`),n&&(n.innerHTML=e.wins),r&&(r.innerHTML=e.losses),i&&(i.innerHTML=`$${e.profit??0}`),a&&(a.innerHTML=e.win_rate>=80?`High`:e.win_rate>=60?`Medium`:`Low`)}catch(e){console.error(`Statistics Error:`,e)}}async function a(){console.log(`🔥 loadTradeHistory() called`);try{let e=await fetch(`https://pocket-option-signal-api-production.up.railway.app/trade/all`);console.log(`Response Status:`,e.status);let t=await e.json();console.log(`Trades received:`,t),console.log(`Trade count:`,t.length);let n=document.getElementById(`historyCount`);n&&(n.textContent=`${t.length} Trades`);let r=document.getElementById(`historyBody`);if(!r){console.error(`Missing element: historyBody`);return}r.innerHTML=``,t.sort((e,t)=>new Date(t.entry_time)-new Date(e.entry_time)),t.slice(0,50).forEach(e=>{let t=document.createElement(`tr`),n=e.entry_time?new Date(e.entry_time).toLocaleTimeString():`--`,i=e.result??`--`,a=i===`--`?`pending`:i.toLowerCase(),o=e.action??`WAIT`;t.innerHTML=`
                ...
                <td>${e.profit??`--`}</td>
                <td>${e.confidence??`--`}%</td>
            `,r.appendChild(t)}),console.log(`Rows inserted:`,r.children.length)}catch(e){console.error(`History Error:`,e)}}var o=null,s=null,c=[];function l(){if(o=document.getElementById(`miniChart`),!o)return;let e=window.devicePixelRatio||1;o.width=o.clientWidth*e,o.height=o.clientHeight*e,s=o.getContext(`2d`),s.scale(e,e)}function u(e){c=Array.isArray(e)?e:[],d()}function d(){if(console.log(`Drawing`,c.length,`candles`),!(!s||!o)){if(s.clearRect(0,0,o.clientWidth,o.clientHeight),f(),p(),c.length===0){s.fillStyle=`#94A3B8`,s.font=`15px Segoe UI`,s.fillText(`Waiting for market data...`,20,35);return}m()}}function f(){let e=s.createLinearGradient(0,0,0,o.clientHeight);e.addColorStop(0,`#101827`),e.addColorStop(1,`#0B1220`),s.fillStyle=e,s.fillRect(0,0,o.clientWidth,o.clientHeight)}function p(){let e=o.clientWidth,t=o.clientHeight;s.strokeStyle=`#1E293B`,s.lineWidth=1;for(let n=0;n<=5;n++){let r=t/5*n;s.beginPath(),s.moveTo(0,r),s.lineTo(e,r),s.stroke()}for(let n=0;n<=8;n++){let r=e/8*n;s.beginPath(),s.moveTo(r,0),s.lineTo(r,t),s.stroke()}}function m(){let e=c.slice(-50),t=e.map(e=>e.high),n=e.map(e=>e.low),r=Math.max(...t),i=Math.min(...n),a=(r-i)*.08,l=r+a-(i-a),u=o.width/e.length;e.forEach((t,n)=>{let r=n*u+u/2,a=o.height-(t.open-i)/l*o.height,c=o.height-(t.close-i)/l*o.height,d=o.height-(t.high-i)/l*o.height,f=o.height-(t.low-i)/l*o.height,p=t.close>=t.open;s.strokeStyle=p?`#22C55E`:`#EF4444`,s.fillStyle=p?`#22C55E`:`#EF4444`,s.beginPath(),s.moveTo(r,d),s.lineTo(r,f),s.stroke();let m=Math.min(a,c),h=Math.max(Math.abs(c-a),2);s.fillRect(r-u*.25,m,u*.5,h),n===e.length-1&&(s.shadowColor=s.fillStyle,s.shadowBlur=12,s.fillRect(r-u*.25,m,u*.5,h),s.shadowBlur=0)})}var h=null;function g(){h&&=(clearInterval(h),null)}function _(e,t){g();let n=document.getElementById(`countdown`),r=document.getElementById(`countdownLabel`),i=document.getElementById(`action`),a=document.getElementById(`entryMessage`);function o(){let t=e();if(t===`WAITING`){n.innerHTML=`--:--`,r.innerHTML=`🟡 WAITING FOR SETUP`,i.innerHTML=`WAIT`,a.innerHTML=`Waiting for a valid trade setup.`;return}if(t===`ANALYZING`){n.innerHTML=`--:--`,r.innerHTML=`🔍 ANALYZING`,i.innerHTML=`WAIT`,a.innerHTML=`Analyzing the current market setup.`;return}if(t===`CONFIRMING`){n.innerHTML=`--:--`,r.innerHTML=`🟡 CONFIRMING`,i.innerHTML=`WAIT`,a.innerHTML=`Waiting for stronger confirmation.`;return}if(t===`READY`){n.innerHTML=`--:--`,r.innerHTML=`🟢 PREPARING ENTRY`,i.innerHTML=`READY`,a.innerHTML=`Setup detected. Waiting for confirmation.`;return}if(t===`WAITING_FOR_CANDLE_CLOSE`){n.innerHTML=`--:--`,r.innerHTML=`⏳ WAITING FOR CANDLE CLOSE`,i.innerHTML=`WAIT`,a.innerHTML=`Analyzing the current candle. Final signal will be confirmed when it closes.`;return}if(t===`ENTRY`){n.innerHTML=`NOW`,r.innerHTML=`🚀 ENTER NOW`,i.innerHTML=`🚀 ENTER NOW`,a.innerHTML=`Final signal confirmed. Enter immediately on the new candle.`;return}if(t===`ACTIVE`){n.innerHTML=`--:--`,r.innerHTML=`🟢 TRADE ACTIVE`,i.innerHTML=`ACTIVE`,a.innerHTML=`Trade is currently running.`;return}if(t===`RESULT`){n.innerHTML=`--:--`,r.innerHTML=`🏁 TRADE COMPLETE`,i.innerHTML=`RESULT`,a.innerHTML=`Trade completed. Waiting for the next setup.`;return}n.innerHTML=`--:--`,r.innerHTML=`⚪ WAITING`,i.innerHTML=`WAIT`,a.innerHTML=`Waiting for the next valid setup.`}o(),h=setInterval(o,1e3)}function v(e){let t=document.getElementById(`instruction`),n=document.getElementById(`reason1`),r=document.getElementById(`reason2`),i=document.getElementById(`reason3`),a=document.getElementById(`instructionStatus`);t&&(t.textContent=e.action===`CALL`?`🟢 BUY CALL AT NEXT CANDLE`:e.action===`PUT`?`🔴 BUY PUT AT NEXT CANDLE`:`🟡 WAIT FOR THE NEXT CANDLE`),n&&(n.textContent=e.reasons?.[0]||``),r&&(r.textContent=e.reasons?.[1]||``),i&&(i.textContent=e.reasons?.[2]||``),a&&(a.textContent=e.market_state||`Monitoring market...`)}function y(e){b(`emaStatus`,e.trend===`BULLISH`?`✓ Bullish`:`✓ Bearish`),b(`emaStrength`,e.grade??`--`),b(`rsiStatus`,e.rsi_status??`--`),b(`rsiStrength`,e.rsi_strength??`--`),b(`macdStatus`,e.macd_status??`--`),b(`macdStrength`,e.macd_strength??`--`),b(`volumeStatus`,e.volume_status??`--`),b(`volumeStrength`,e.volume_strength??`--`),b(`structureStatus`,e.structure_status??`--`),b(`structureStrength`,e.structure_strength??`--`),b(`volatilityStatus`,e.volatility_status??`--`),b(`volatilityStrength`,e.volatility_strength??`--`),b(`supportStatus`,e.support_status??`--`),b(`supportStrength`,e.support_strength??`--`),b(`liquidityStatus`,e.liquidity_status??`--`),b(`liquidityStrength`,e.liquidity_strength??`--`)}function b(e,t){let n=document.getElementById(e);n&&(n.textContent=t)}function x(e={},t={}){let n=e.asset||t.asset||`---`,r=Array.isArray(t.candles)?t.candles:[],i=r.length>0?r[r.length-1]:null,a=e.entry_price??i?.close??0;S(`asset`,n),S(`trend`,e.trend),S(`risk`,e.risk),S(`expiration`,e.expiration),S(`probability`,`${Number(e.probability??0).toFixed(1)}%`),S(`grade`,e.grade),S(`session`,e.session),S(`regime`,e.regime),S(`chartAsset`,n),S(`chartPrice`,Number(a).toFixed(5));let o=document.getElementById(`chartChange`);if(o){let t=e.trend||`---`;o.textContent=`${t} • ${e.session||`---`}`,o.className=`market-change`,t===`BULLISH`?o.classList.add(`bullish`):t===`BEARISH`?o.classList.add(`bearish`):o.classList.add(`neutral`)}}function S(e,t){let n=document.getElementById(e);if(!n)return;n.textContent=t??`---`,n.classList.remove(`BULLISH`,`BEARISH`,`SIDEWAYS`,`LOW`,`MEDIUM`,`HIGH`);let r=[`BULLISH`,`BEARISH`,`SIDEWAYS`,`LOW`,`MEDIUM`,`HIGH`],i=String(t??``).toUpperCase();r.includes(i)&&n.classList.add(i)}function C(e){w(e),T(e),E(e),D(e)}function w(e){let t=document.getElementById(`action`);if(!t)return;let n=String(e.action||`WAIT`).toUpperCase(),r=String(e.bias||``).toUpperCase(),i=String(e.market_state||``).toUpperCase(),a=String(e.trade_status||``).toUpperCase(),o=n;switch((i===`ACTIVE`||a===`ACTIVE`)&&n===`WAIT`&&(r===`CALL`||r===`PUT`)&&(o=r),t.textContent=o,t.className=`signal-action`,o){case`CALL`:t.classList.add(`call`);break;case`PUT`:t.classList.add(`put`);break;default:t.classList.add(`wait`);break}}function T(e){let t=document.getElementById(`confidence`);if(!t)return;let n=Number(e.confidence);Number.isFinite(n)?t.textContent=`${Math.round(n)}%`:t.textContent=`--`}function E(e){let t=document.getElementById(`trend`),n=document.getElementById(`risk`),r=document.getElementById(`expiration`);t&&(t.textContent=e.trend??`--`),n&&(n.textContent=e.risk??`--`),r&&(r.textContent=e.expiration??`--`)}function D(e){let t=document.getElementById(`signalStatus`);if(!t)return;let n=String(e.market_state||``).toUpperCase(),r=String(e.trade_status||``).toUpperCase(),i=String(e.bias||``).toUpperCase();if((n===`ACTIVE`||r===`ACTIVE`)&&(i===`CALL`||i===`PUT`)){t.textContent=`ACTIVE TRADE — ${i}`,t.className=`active`;return}if(e.reason&&String(e.reason).trim()!==``){t.textContent=e.reason;return}t.textContent=e.market_state||`Waiting...`}function O(e){let t=document.getElementById(`status`),n=document.getElementById(`statusText`),r=document.getElementById(`updated`),i=document.getElementById(`engineStatus`);!t||!n||(e?(t.textContent=`🟢 Online`,t.className=`status online`,n.textContent=`🟢 Connected`,n.className=`online`,r&&(r.textContent=new Date().toLocaleTimeString()),i&&(i.textContent=`Running`)):(t.textContent=`🔴 Offline`,t.className=`status offline`,n.textContent=`🔴 Disconnected`,n.className=`offline`,i&&(i.textContent=`Offline`)))}async function k(){return new Promise(e=>{chrome.runtime.sendMessage({type:`GET_STATE`},t=>{if(chrome.runtime.lastError){console.error(`Extension state error:`,chrome.runtime.lastError.message),e({});return}e(t||{})})})}async function A(){console.log(`Dashboard initialized`);try{l(),console.log(`Chart initialized`),await j(),console.log(`First refresh complete`),_(()=>window.marketState,()=>window.latestCandle),console.log(`Countdown started`),setInterval(j,1e3)}catch(e){console.error(`Dashboard Error:`,e)}}async function j(){let e,o;try{e=await t(),O(!0)}catch(t){O(!1),console.error(`API Connection Error:`,t),e={status:`No signal yet`}}try{o=await k()}catch(e){console.error(`Unable to get extension market state:`,e),o={}}let s=e?.asset||o.marketAsset||null,c=Array.isArray(o.marketCandles)?o.marketCandles:[];console.log(`======================================`),console.log(`LIVE MARKET STATE`),console.log(`Asset:`,s),console.log(`Candles:`,c.length),console.log(`======================================`),window.marketState=e?.market_state||`WAITING`;try{C(e),v(e),y(e)}catch(e){console.error(`AI UI update error:`,e)}try{x(e,{asset:s,candles:c})}catch(e){console.error(`Market UI update error:`,e)}try{if(c.length===0&&s){console.log(`No local candles yet. Requesting backend candles for:`,s);try{c=await n(s)}catch(e){console.warn(`Backend candle request failed:`,e)}}c.length>0?(window.latestCandle=c[c.length-1],console.log(`Latest candle:`,window.latestCandle),console.log(`Total candles:`,c.length),u(c)):(window.latestCandle=null,console.warn(`No candles available yet.`))}catch(e){console.error(`Candle UI error:`,e)}try{r(Number(e?.confidence??0))}catch(e){console.error(`Confidence UI error:`,e)}if(window.tradeRefreshTimer||(window.tradeRefreshTimer=Date.now()),Date.now()-window.tradeRefreshTimer>5e3){try{await i(),await a()}catch(e){console.error(`Trade data refresh error:`,e)}window.tradeRefreshTimer=Date.now()}}var M=`https://pocket-option-signal-api-production.up.railway.app`,N=`pocketOptionAuthToken`,P=`pocketOptionUser`;async function F(){return(await chrome.storage.local.get(N))[N]||null}async function I(e,t){let n=await fetch(`${M}/auth/login`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({email:e,password:t})}),r=await n.json();if(!n.ok)throw Error(r.detail||`Unable to login.`);return await chrome.storage.local.set({[N]:r.token,[P]:r.user}),r}async function L(e,t){let n=await fetch(`${M}/auth/register`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({email:e,password:t})}),r=await n.json();if(!n.ok)throw Error(r.detail||`Unable to create account.`);return r}async function R(){let e=await F();if(!e)return null;try{let t=await fetch(`${M}/auth/me`,{method:`GET`,headers:{Authorization:`Bearer ${e}`}});if(!t.ok)return await z(),null;let n=await t.json();return await chrome.storage.local.set({[P]:n.user}),n.user}catch(e){return console.error(`Session verification failed:`,e),null}}async function z(){let e=await F();if(e)try{await fetch(`${M}/auth/logout`,{method:`POST`,headers:{Authorization:`Bearer ${e}`}})}catch(e){console.warn(`Logout request failed:`,e)}await chrome.storage.local.remove([N,P])}function B(e){if(document.getElementById(`authScreen`))return;let t=document.createElement(`div`);t.id=`authScreen`,t.innerHTML=`
